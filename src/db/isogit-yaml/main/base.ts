  ...
  public async init(forceReset = false) {
    ...
    try {
      if (forceReset === true) {
        ...
        doInitialize = true;
      } else if (!(await this.git.isUsingRemoteURLs({ origin: `${this.opts.repoURL}.git` }))) {
        log.warn("C/db/isogit-yaml: Git has mismatching remote URL, reinitializing with:", this.opts.repoURL);
        doInitialize = true;
      } else {
